...
# Source-build controls (space-separated list; currently supports: openssl)
OCS_SRC_BUILDS="${OCS_SRC_BUILDS:-openssl}"
...
if ! dnf --forcearch="$ARCH" --version >/dev/null 2>&1; then
    if [ -z "${OCS_IN_CONTAINER:-}" ] && [ "${OCS_USE_DOCKER:-1}" != "0" ] && command -v docker >/dev/null 2>&1; then
        ...
        docker run --rm --platform="$OCS_DOCKER_PLATFORM" --entrypoint /usr/bin/env -v "$PWD:/work" -w /work \
            ...
            LANG=C \
            LC_ALL=C \
            /bin/bash --noprofile --norc -c "dnf -y install wget curl-minimal python3 tar findutils --allowerasing && ./scripts/build_ocs.sh"
        exit $?
    fi
